(function() {
  ...
  const COMBO_DEALS_PATHS = [
    'combo-offers.json',
    '/combo-offers.json'
  ];

  async function loadData() {
    if (categoryId === 'combo-deals') {
      for (const path of COMBO_DEALS_PATHS) {
        try {
          ...
          console.error('Failed to load combo deals from path:', path, e);
        }
      }
      throw new Error('Unable to load combo deals data');
    }
      ...
      grid.addEventListener('click', function(event) {
        ...
        if (addToCartButton) {
            ...
            if (window.CartManager && typeof window.CartManager.addToCart === 'function') {
                window.CartManager.addToCart(productId, 1, { theme: 'default' });
