document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderSummary(items) { /* patched by Rovo Dev */
        let subtotal = items.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
        
        // Cart is considered a test cart only if every item is a test product
        ...
        // Enforce a flat 1 GHS total when only test products are in cart
        ...
        if (isOnlyTestProductCart) {
            subtotal = 1; // Display subtotal as 1 GHS for clear UX
        }
