...
function getShippingCost() {
  try {
    // Waive delivery fees for special test product-only orders
    if (Array.isArray(cartItemsCache) && cartItemsCache.length > 0) {
      const isOnlyTest = cartItemsCache.every(it => (it.product?.id || it.productId || it.id) === 'storage-devices-016-test');
      if (isOnlyTest) return 0;
    }
  } catch(e) {}
...
function renderSummaryTotals(items) {
    ...
    container.innerHTML = `
        ...
        <div class="summary-row">
            <span>Shipping (${SHIPPING_ZONES[selectedShippingZone].name}):</span>
