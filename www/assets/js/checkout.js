...
/**
 * Checks if cart contains Starlink products
 * @return {boolean} True if cart contains Starlink products
 */
function hasStarlinkProducts() {
  return cartItemsCache.some(item => {
    const product = item.product || item;
    const id = String(product.id || product._id || product.productId || '');
    return id.startsWith('starlink-');
  });
}

/**
 * Gets cost for currently selected shipping zone.
 */
...
async function placeOrder() {
    ...
    try {
        ...
        // Skip test products (used for payment gateway testing) and only check real products
        for (const item of cartItemsCache) {
            const product = item.product || item;
            if (isTestProduct(product)) {
                continue; // do not attempt to fetch stock for test products
            }
            // Prefer slug for backend lookup; fall back to id/productId, then _id last
            const lookupKey = product.slug || product.id || product.productId || product._id;
            if (!lookupKey) {
                throw new Error('Invalid cart item: missing product identifier');
            }
            const res = await fetch(`${API_BASE_URL}/products/${encodeURIComponent(lookupKey)}`);
            if (!res.ok) throw new Error('Failed to fetch product details');
            const prodData = await res.json();
            const qty = item.quantity || item.qty || 1;
            if (Number(prodData.stock) < Number(qty)) {
        ...
        // Validate payment method for Starlink products
        const hasStarlink = hasStarlinkProducts();
        if (hasStarlink && paymentMethod !== 'paystack') {
            throw new Error('Starlink products require direct payment via Paystack. Please select Paystack as your payment method.');
        }
            ...
            if (typeof CartManager !== 'undefined' && CartManager.clearCart) {
                ...
                await fetch(`${API_BASE_URL}/cart`, {
                    ...
                    if (typeof console !== 'undefined' && console.warn) {
                        console.warn('Failed to clear cart via API:', err);
                    }
                });
            }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 5.5. Enforce Paystack payment for Starlink products
    const hasStarlink = hasStarlinkProducts();
    if (hasStarlink) {
        // Disable COD option
        ...
        const paystackOption = document.querySelector('.payment-option[data-payment="paystack"]');
        if (paystackOption) {
            paystackOption.classList.add('selected');
            const paystackRadio = paystackOption.querySelector('input[type="radio"]');
            if (paystackRadio) {
                paystackRadio.checked = true;
                paystackRadio.disabled = false;
            }
        }
        ...
        if (paymentSection) {
            ...
            warningMsg.style.cssText = 'background: rgba(255, 193, 7, 0.1); border: 1px solid rgba(255, 193, 7, 0.3); color: #856404; padding: 12px; border-radius: 6px; margin-bottom: 15px; font-size: 14px;';
            warningMsg.innerHTML = '<strong><i class="fas fa-info-circle" style="margin-right: 8px;"></i>Starlink products require direct payment</strong><br>Starlink products can only be purchased using Paystack (Card, Mobile Money, Apple Pay). Cash on Delivery is not available for Starlink products.';
            paymentSection.insertBefore(warningMsg, paymentSection.querySelector('.payment-option'));
        }
    }
