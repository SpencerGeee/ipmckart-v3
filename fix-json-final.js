...
async function regenerateWithCorrectStructure() {
  ...
  // Group by category, then subcategory as ARRAYS
  const categories = [];
  const categoryMap = {};
  ...
  activeProducts.forEach(p => {
    ...
    // Find or create subcategory (as object for lookup, but we'll store as array)
  ...
  // Convert to final structure with categories as array
  ...
  console.log('[2/12] Generating products.grouped.json...');
  const groupedFlat = {};
  categories.forEach(cat => {
    cat.subcategories.forEach(sub => {
  ...
  console.log('[3/12] Generating products.flat2.json...');
  ...
  console.log('[4/12] Generating flash-sales.json...');
  ...
  console.log('[5/12] Generating black-friday.json...');
  ...
  console.log('[6/12] Generating christmas-sale.json...');
  ...
  console.log('[7/12] Generating back-to-school.json...');
  ...
  console.log('[8/12] Generating top-selling.json...');
  ...
  console.log('[9/12] Generating raw-products.json...');
  ...
  console.log('[10/12] Generating db-products-export.json...');
  ...
  console.log('[11/12] Generating assets/data files...');
  ...
  if (testRead.categories && Array.isArray(testRead.categories)) {
    ...
    if (testRead.categories.length > 0 && testRead.categories[0].subcategories) {
      console.log(`   ✅ Verified: first category has subcategories array with ${testRead.categories[0].subcategories.length} items`);
    }
  } else {
  ...
  console.log('  ✓ Subcategories as arrays (not objects)');
